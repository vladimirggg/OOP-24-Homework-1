...
MultiSet::MultiSet(unsigned int n, unsigned int k) : maxNumber(n) {
    if(k < 1 || k > 8) throw "K out of bounds!"; //TODO find a way to skip the exception
    else bucketSize = k;
...
void MultiSet::add(unsigned int number) {
    if(number > maxNumber) throw "Number out of bounds!";
...
void MultiSet::printNumbers() const {
    for (size_t i = 1; i <= maxNumber; i++)
        if(occurrenceCount(i)) std::cout<<i<<std::endl;
}
...
MultiSet MultiSet::intersection(const MultiSet& other) const {
    MultiSet intersection(std::min(maxNumber,other.maxNumber), 
                        std::min(bucketSize, other.bucketSize));
    const size_t arraySize = intersection.getArraySize();
    for (size_t i = 0; i < arraySize; i++) {
        intersection.numberSet[i] = (numberSet[i] & other.numberSet[i]);
    }
    return intersection;
}

MultiSet MultiSet::difference(const MultiSet& other) const {
    MultiSet difference(std::min(maxNumber,other.maxNumber), 
                        std::min(bucketSize, other.bucketSize));
    const size_t arraySize = difference.getArraySize();
    for (size_t i = 0; i < arraySize; i++) {
        difference.numberSet[i] = (numberSet[i] ^ other.numberSet[i]);
    }
    return difference;
}
